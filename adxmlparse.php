...
enable_qty_increments','product_name','store_id','product_type_id','product_status_changed','product_changed_websites','url');
...
foreach ($mainXml->shop[0]->offers[0] as $offer) {
    //if ($i < 100) {
        ...
        $content = array('admin','base','Default','simple',$categories, $sku,'0',$name,'','','use config','Use config','','',$image,$image,$image,'','No layout updates','Block agter Info Column',
                         ...
                         '','','1','0','0','1','1','0','1','0','0','1','1',$name,'0','simple',$url);
        fputcsv($fp, $content);
    //}
}
fclose($fp);
